...
for i in range(len(df_train)):
    ...
    stars = int(bool(reg.search(line)))
    star_list.append(stars)

    # number of bad words in the text
    bad = 0
    for word in bad_ugrams:
        bad += len(re.findall(r'{0}'.format(word), line))
    bad_list.append(bad)
...
f_out = open('../output/test_1.csv', 'w')
f_out.write("Id,Category\n")


print('Writing to file... \n')
import re
f_out_only0s = open('../analysis/normal_0s.txt','w')
